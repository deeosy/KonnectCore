...
const addTimestamps = (items) =>
  items.map((i) => ({ ...i, ts: new Date(i.date || i.createdAt || 0).getTime() }))
  ...
  try {
    ...
    const [collections, payments, loans, visits] = await Promise.all([
      ...
      Payment.find({ memberId: member._id })
        .select('type amount method status receiptNumber paymentDate description')
        .populate('processedBy', 'name')
        .lean(),
      Loan.find({ memberId: member._id })
        .select('type amount balance status dueDate createdAt')
        .lean(),
