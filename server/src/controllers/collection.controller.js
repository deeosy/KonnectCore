...
export const createCollection = async (req, res, next) => {
  try {
    ...
    // configured default crop price. Note: the fallback reads
    // req.user.settings?.defaultPrices, but the User model has no settings
    // field, so in practice this resolves to undefined and the price is 0 —
    // the organisation's defaultCropPrices map is never consulted here.
    const price =
      data.pricePerUnit || req.user.settings?.defaultPrices?.[data.crop] || 0;
