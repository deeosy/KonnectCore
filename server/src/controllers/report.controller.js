...
export const memberReport = async (req, res, next) => {
  try {
    ...
    if (search) filter.$or = [{ firstName: { $regex: search, $options: 'i' } }, { lastName: { $regex: search, $options: 'i' } }, { phone: { $regex: search, $options: 'i' } }, { membershipNumber: { $regex: search, $options: 'i' } }]
...
export const collectionReport = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, count: collections.length, totals, data: collections })
  } catch (error) {
    next(error)
  }
}
  ...
  try {
    ...
    const summary = await Promise.all(
      groups.map(async (g) => {
        ...
        const collections = await Collection.find({ memberId: { $in: memberIds } })
        ...
        return {
          ...
          totalHarvestValue: collections.reduce((s, c) => s + (c.totalValue || 0), 0),
...
export const expenseReport = async (req, res, next) => {
  try {
    const expenses = await Expense.find().populate('createdBy', 'name').sort('-date').lean()
