// MemberNew.jsx - Create/edit member form (edit detected via route :id param or ?id query).
// Calls GET /groups and GET /users for the group/officer dropdowns, GET /members/:id on edit,
// POST /members on create, and PUT /members/:id on edit. Sends data as multipart/form-data to
// support an optional photo upload. Membership number is auto-generated by the server if blank.
...
// Empty baseline form used for create mode and as the starting shape for edit mode.
const emptyForm = {
  firstName: "",
  lastName: "",
  phone: "",
...
// FormSection - animated card wrapper that gives each form section a staggered entrance.
...
// MemberNew - the create/edit member page component.
// editId comes from the URL :id param or a ?id query string; isEdit distinguishes
// create-mode from edit-mode so the same form can be reused for both.
export default function MemberNew() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {});
    if (editId) {
      // Edit mode: prefill the form from GET /members/:id. Populated groupId/officer
      // are objects from the server, so unwrap _id when present.
      api
        .get(`/members/${editId}`)
        .then(({ data }) => {
          const m = data.data;
  ...
  const set = (key, value) => {
    ...
    // Clear the field's error once the user corrects it.
    setErrors((e) => (e[key] ? { ...e, [key]: "" } : e));
  };

  // handleCropToggle - add/remove a crop from the mainCrops array (toggle pill buttons).
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Run client-side validation and block submit if any field fails.
    const nextErrors = {
    ...
    // native array type; the server normalizes that JSON string back into a
    // real array (see normalizeCrops in member.controller.js).
